...
class MazeView2D:
    ...
    def __init__(self, maze_name="Maze2D", maze_file_path=None,
                 ...
                 has_loops=False, num_portals=0, num_rewards=0, enable_render=True):
        ...
        if maze_file_path is None:
            ...
            self.__maze = Maze.load_maze(maze_file_path)
            # self.__maze = Maze(maze_cells=Maze.load_maze(maze_file_path).maze_cells, maze_rewards=Maze.load_maze(maze_file_path).maze_rewards)
    ...
    def __draw_rewards(self, transparency=80):  # TODO: Draw better colours for rewards?

        if self.__enable_render is False:
            return

        colour_range = np.linspace(0, 255, len(self.maze.rewards), dtype=int)
        colour_i = 0
        for reward in self.maze.rewards:
            colour = ((100 - colour_range[colour_i]) % 255, colour_range[colour_i], 0)
            colour_i += 1
            self.__colour_cell(reward, colour=colour, transparency=transparency)
    ...
    def __colour_cell(self, cell, colour, transparency):
        ...
        x = int(cell[0] * self.CELL_W + 1.5)  # + 0.5 + 1)
        y = int(cell[1] * self.CELL_H + 1.5)  # + 0.5 + 1)
        w = int(self.CELL_W - 1.5)  # + 0.5 - 1)
        h = int(self.CELL_H - 1.5)  # + 0.5 - 1)
...
class Maze:
    ...
    def __init__(self, maze_cells=None, maze_size=(10, 10), has_loops=True, num_portals=0, num_rewards=0,
                 maze_rewards=None):
    ...
    def save_maze(self, file_path):
        ...
        if not os.path.exists(os.path.dirname(file_path)):
            ...
            # rewards_array = np.array([list(reward) for reward in self.rewards])
            # print(rewards_array)
            # saved_data_array = np.stack(self.maze_cells, rewards_array)
    ...
    def _generate_maze(self):
        ...
        if self.num_rewards > 0:
            self.__set_random_rewards(num_reward_sets=self.num_rewards)
    ...
    def __set_random_rewards(self, num_reward_sets):
        # find some random cells to break
        num_reward_sets = int(num_reward_sets)

        # limit the maximum number of reward sets to the number of cells available.
        num_reward_sets = min(self.MAZE_W * self.MAZE_H, num_reward_sets)

        # the first cell is reserved (start point)
        cell_ids = random.sample(range(1, self.MAZE_W * self.MAZE_H), num_reward_sets)

        for i in range(num_reward_sets):
            # remove the cell for the set of potential cell_ids
            current_reward_id = cell_ids.pop()
            # convert reward id to location
            x = current_reward_id % self.MAZE_H
            y = int(current_reward_id / self.MAZE_H)
            # append the new reward to the maze
            self.__rewards.append((x, y))
    ...
    def is_reward(self, cell):
        return tuple(cell) in self.__rewards
    ...
    @property
    def rewards(self):
        return tuple(self.__rewards)

    def on_reward(self, cell):
        for reward in self.__rewards:
            if np.array_equal(cell, reward):
                return True
        return False
